...
namespace LibRecSysCS.Relative
{
    ...
    public class PrefRelations
    {
        ...
        SortedDictionary<int, SparseMatrix> preferenceRelations;
        private Matrix<double> userSimilarities;

        public SortedDictionary<int, SparseMatrix> PreferenceRelationsByUser
        {
            get
            {
        ...
        {
            preferenceRelations = new SortedDictionary<int, SparseMatrix>();
            ItemCount = itemCount;
        }
        ...
        public static PrefRelations CreateDiscrete(DataMatrix R,  bool ignoreEqual = false)
        {
            ...
            Parallel.ForEach(R.Users, user =>
            {
                    ...
                    foreach (Tuple<int, double> right in userRatings.Ratings)
                    {
                        ...
                        if (leftItemRating > rightItemRating)
                        {
                            userPreferencensCache.Add(new Tuple<int, int, double>(
                                leftItemIndex, rightItemIndex, Constants.Preferences.Preferred));
                        }
                        else if (leftItemRating < rightItemRating)
                        {
                            userPreferencensCache.Add(new Tuple<int, int, double>(
                                leftItemIndex, rightItemIndex, Constants.Preferences.LessPreferred));
                        }
                        else // i.e. leftItemRating==ratingRight
                        {
                            ...
                            {
                                userPreferencensCache.Add(new Tuple<int, int, double>(
                                    leftItemIndex, rightItemIndex, Constants.Preferences.EquallyPreferred));
                            }
                        }
                    }
                }
                ...
                if(!ignoreEqual) Debug.Assert(debug1 == debug2);
        ...
        public static Vector<double> PreferencesToPositions(SparseMatrix userPreferences)
        {
            ...
            SparseVector preferredCountByItem = SparseVector.OfEnumerable(userPreferences.FoldByRow((count, pref) =>
                    count + (pref == Constants.Preferences.Preferred ? 1 : 0), 0.0));

            SparseVector lessPreferredCountByItem = SparseVector.OfEnumerable(userPreferences.FoldByRow((count, pref) =>
                    count + (pref == Constants.Preferences.LessPreferred ? 1 : 0), 0.0));
            ...
            SparseVector equallyPreferredCountByItem = SparseVector.OfEnumerable(userPreferences.FoldByRow((count, pref) =>
                    count + (pref == Constants.Preferences.EquallyPreferred  ? 1 : 0), 0.0));
     ...
     .PointwiseDivide(lessPreferredCountByItem + preferredCountByItem + equallyPreferredCountByItem) + Constants.Preferences.PositionShift;
            ...
            double debug = Math.Abs(positionByItem.Sum() - ((SparseVector)positionByItem).NonZerosCount * Constants.Preferences.PositionShift);
            //Debug.Assert(debug < 0.001);
                ...
                */
                ...
                Vector<double> preferredCountByItem = DenseVector.OfArray(
                    userPreferences.FoldByRow((count, rating) =>
                        count + (rating == Constants.Preferences.Preferred ? 1 : 0), 0.0));
                Vector<double> lessPreferredCountByItem = DenseVector.OfArray(
                userPreferences.FoldByRow((count, rating) =>
                count + (rating == Constants.Preferences.LessPreferred ? 1 : 0), 0.0));
                Vector<double> equallyPreferredCountByItem = DenseVector.OfArray(
userPreferences.FoldByRow((count, rating) =>
count + (rating == Constants.Preferences.EquallyPreferred ? 1 : 0), 0.0));
        ...
        public void quantization(double min, double range, List<double> quantizer)
        {
            ...
            preferenceRelations = new SortedDictionary<int,SparseMatrix>(preferenceRelationsquantizationd);
        }
    }
}
